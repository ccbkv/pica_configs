class Happy extends ComicSource {
    ...
    // 通用评论加载功能
    async loadCommentsCommon(comicId, epId, page, replyTo, from) {
        if (replyTo) {
            // 加载楼中楼评论列表
            ...
            return {
                comments: data.data.items.map(item => this.parseComment(item)),
    ...
    // 带缓存的章节列表加载功能
    async loadChaptersWithCache(comicId) {
        // 获取单页章节数据
    ...
    explore = [{
        ...
        load: async () => {
            ...
            for (const part of parts) {
                ...
                const comics = part.querySelectorAll(".manga-cover").map(item => this.parseHtmlComic(item))
    ...
    categoryComics = {
        ...
        load: async (category, param, options, page) => {
            ...
            const data = JSON.parse(res.body)
            return {
                comics: data.data.items.map(item => this.parseJsonComic(item)),
                maxPage: data.data.isEnd ? page : null
            }
        },
        ...
        ranking: {
            ...
            load: async (option, page) => {
                ...
                const doc = new HtmlDocument(res.body)
                const comics = doc.querySelectorAll(".manga-rank").map(item => this.parseHtmlComic(item))
                doc.dispose()
    ...
    comic = {
        ...
        loadInfo: async (id) => {
            ...
            // 从HTML提取JSON数据
            const matchResult = res.body.match(/<mip-data>\s*<script type="application\/json">\s*([\S\s]*?)<\/script>\s*<\/mip-data>/i)
            const jsonInHtml = matchResult ? matchResult[1] : null
            const comicData = jsonInHtml ? JSON.parse(jsonInHtml) : {}
            ...
            const recommend = doc.querySelectorAll(".manga-cover").map(item => this.parseHtmlComic(item))

            // 从内嵌JSON解析信息
            const stars = comicData.score ? parseFloat(comicData.score) || null : null
