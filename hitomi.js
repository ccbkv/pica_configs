class HitomiJs extends ComicSource{
  ...
  _mapGalleryBlockInfoToComic(n){
    // 正确格式化标签以支持翻译
    return {id:n.gid,title:n.title,subTitle:n.artists.length?n.artists.join(" "):"",cover:get_thumbnail_url_from_hash(n.thumbnail_hashs[0],true),tags:[...n.series.map(m=>"parody:"+m),...n.females.map(m=>"female:"+m),...n.males.map(m=>"male:"+m),...n.others.map(m=>"tag:"+m)],language:n.language,description:n.type?n.type+"\n"+formatDate(n.posted_time):formatDate(n.posted_time)};
  }
  init(){}
...
function parseGalleryBlockInfo(body){
  ...
  const titleLink=mangaEl.querySelector("h1.lillie > a")||mangaEl.querySelector("a.lillie");
  const href=titleLink?titleLink.attributes["href"]:"";
  const m=/-(\d+)\.html$/.exec(href);
  const gid=m?m[1]:"0";
  const title=titleLink?titleLink.text:(mangaEl.querySelector("h1.lillie")?.text||"");
  const thumbnail_hashs=[];
  const srcs=Array.from(mangaEl.querySelectorAll("img")).map(a=>((a.attributes["data-src"])||(a.attributes["src"])||"").trim()).filter(s=>s);
  srcs.forEach(src=>{const r=/\/(\w{64})\./.exec(src);if(r){const hash=r[1];thumbnail_hashs.push(hash);}});
  const artists=Array.from(mangaEl.querySelectorAll(".artist-list li a")).map(a=>a.text.trim());
  let language=undefined;let series=[];let type=undefined;
  const rows=mangaEl.querySelectorAll(".dj-desc tr");
  rows.forEach(row=>{const key=row.children[0].text.trim().toLowerCase();const valueCell=row.children[1];switch(key){case "series":{const text=valueCell.text.trim();if(text!=="N/A"){const as=valueCell.querySelectorAll("a");as.forEach(a=>series.push(a.text.trim()));} break;} case "type":{type=valueCell.text.trim();break;} case "language":{if(valueCell.querySelector("a")){const href=valueCell.querySelector("a").attributes["href"];const r=/\/index-(\w+)\.html/.exec(href);if(r){language=r[1];}} break;}}});
  const females=[];const males=[];const others=[];
  // 使用正确的标签格式以支持翻译
  Array.from(mangaEl.querySelectorAll(".relatedtags li a")).map(a=>{const text=a.text.trim();if(text.endsWith(" ♀")){females.push(text.slice(0,-2));} else if(text.endsWith(" ♂")){males.push(text.slice(0,-2));} else {others.push(text);} });
  const dateEl=mangaEl.querySelector(".date");
  const posted_time=dateEl?new Date(toISO8601(dateEl.text.trim())):new Date();
...
function parseGalleryDetail(text){
  ...
  return {gid:parseInt(data.id),title:data.title,url:"https://hitomi.la"+data.galleryurl,type:data.type,length:data.files.length,language:("language" in data&&data.language)?data.language:undefined,artists,groups,series,characters,females,males,others,thumbnail_hash:data.files[0].hash,files:data.files,posted_time:new Date(toISO8601(data.date)),translations,related_gids};
}